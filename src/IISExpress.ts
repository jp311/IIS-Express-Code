...
export class IIS {
  ...
  public startWebsite(options?: settings.Isettings) {
    ...
    this._args.protocol = options.protocol
      ? options.protocol
      : settings.protocolType.http;
    ...
    var siteName =
      path.basename(vscode.workspace.rootPath as string) + "-" + uuidV4();

    //If user is using HTTPS & port not in range of auto-approved port numbers (44300-44399)
    //Then display an error & stop process
    if (
      this._args.protocol === settings.protocolType.https &&
      (this._args.port >= 44300 && this._args.port <= 44399) === false
    ) {
      //Using HTTPS but not using a port within the range that supports SSL
      vscode.window.showErrorMessage(
        "When using HTTPS you need to use ports 44300 - 44399 in .vscode/iisexpress.json"
      );
    ...
    try {
      process.execFileSync(this._iisAppCmdPath, [
        "add",
        "site",
        `-name:${siteName}`,
        `-bindings:${this._args.protocol}://localhost:${this._args.port},${this._args.protocol}://127.0.0.1:${this._args.port}`,
        `-physicalPath:${this._args.path}`
      ]);
    } catch (error) {
      console.log(error);
    }
    ...
    var appPool =
      this._args.clr === settings.clrVersion.v40
  ...
  public openWebsite(options?: settings.Isettings) {
    ...
    if (options && options.url) {
      ...
      let startUrl = options.url.startsWith("/")
        ? options.url.substring(1)
        : options.url;

      //Start browser with start url
      process.exec(
        `start ${this._args.protocol}://localhost:${this._args.port}/${startUrl}`
      );
    } else {
      //Uses the 'start' command & url to open default browser
      process.exec(
        `start ${this._args.protocol}://localhost:${this._args.port}`
      );
    }
  }
